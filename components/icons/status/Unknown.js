...
export default class Unknown extends Component {
  ...
  render () {
    ...
    if (inverse) {
      ...
      detailColor = colorForIndex('unknown');
    } else {
      fill = colorForIndex('unknown');
      detailColor = colorForIndex('colored');
    }
    let detail;
    if ('small' !== size) {
      detail = (
        <G>
          <Path fill="none" strokeWidth="2" strokeLinecap="butt" stroke={detailColor}
          ...
          <Circle stroke="none" fill={detailColor} cx="12" cy="17.6" r="1" />
        </G>
      );
    } else {
